	...
	void CalculatePassengerMovement(Vector3 velocity) {
		...
		if (velocity.y != 0) {
			...
			for (int i = 0; i < verticalRayCount; i ++) {
				...
				RaycastHit2D hit = Physics2D.Raycast(rayOrigin, Vector2.up * directionY, rayLength, Passengers);

				if (hit && hit.distance != 0) {
					if (!movedPassengers.Contains(hit.transform)) {
						movedPassengers.Add(hit.transform);
						float pushX = (directionY == 1)?pushX = velocity.x:pushX = 0;
		...
		if (velocity.x != 0) {
			...
			for (int i = 0; i < horizontalRayCount; i ++) {
				...
				RaycastHit2D hit = Physics2D.Raycast(rayOrigin, Vector2.right * directionX, rayLength, Passengers);
		...
		if (directionY == -1 || (velocity.y == 0  && velocity.x != 0)) {
			...
			for (int i = 0; i < verticalRayCount; i ++) {
				...
				RaycastHit2D hit = Physics2D.Raycast(rayOrigin, Vector2.up, rayLength, Passengers);
